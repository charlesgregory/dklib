...
struct kmem_t
{
    ...
    {
        header_t *p, q;
        for (p = this.core_head; p != null;) {
            q = p.ptr;
            free(p);
            p = q;
        }
    }
    header_t *morecore(size_t nu) @nogc
    {
        header_t *q;
        size_t bytes;
        size_t *p;
        ...
        q.ptr = this.core_head, q.size = nu, this.core_head = q;
        p = cast(size_t*)(q + 1);
        *p = nu - 1; /* the size of the free block; -1 because the first unit is used for the core header */
        this.kfree(p + 1); /* initialize the new "core"; NB: the core header is not looped. */
        return this.loop_head;
    }

    void kfree(void *ap) @nogc /* kfree() also adds a new core to the circular list */
    {
        header_t *p, q;
        kmem_t *km = &this;
    ...
    void * kmalloc(size_t n_bytes) @nogc
    {
        ...
        for (p = q.ptr;; q = p, p = p.ptr) { /* search for a suitable block */
            ...
            if (p == km.loop_head) { /* then ask for more "cores" */
                if ((p = this.morecore(n_units)) == null) return null;
            }
        }
    }
    ...
    void *kcalloc(size_t count, size_t size) @nogc
    {
        ...
        p = this.kmalloc(count * size);
        memset(p, 0, count * size);
        return p;
    }

    void *krealloc(void *ap, size_t n_bytes) @nogc // TODO: this can be made more efficient in principle
    {
        kmem_t *km = &this;
        size_t n_units; 
        size_t *p, q;
...
/// All functions below kept source-level homology
...
deprecated("kept for source-level homology; free(km) will likely SIGSEGV when called as km_destroy(&this)")
